    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                newBeliefs[p] = self.beliefs[p] * emissionModel[util.manhattanDistance(p, pacmanPosition)]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(0, numParticlesLeft):
    ...
    def observe(self, observation, gameState):
        ...
        sampleWeights = util.Counter()

        # Jail Case
        if noisyDistance == None:
            self.particleList = [self.getJailPosition()] * len(self.particleList)
        else:
            # Get Weighted samples
            for particle in self.particleList:
                sampleWeights[particle] += emissionModel[util.manhattanDistance(pacmanPosition, particle)]
            # Resample
            if sampleWeights.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                self.particleList = map(lambda _: util.sample(sampleWeights), self.particleList)
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particleList = map(lambda oldPos: self.getSampleFromOldPos(gameState, oldPos) , self.particleList)
    ...
    def getSampleFromOldPos(self, gameState, oldPos):
        return util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        listPossibleGhostPositions = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(listPossibleGhostPositions)
        self.particles = [listPossibleGhostPositions[i % len(listPossibleGhostPositions)] for i in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        # Move the necessary ghosts to jail
        self.moveNecessaryGhostsToJail(noisyDistances)
        ...
        for particle in self.particles:
            weight = 1
            for ghostIndex, noisyDistance in enumerate(noisyDistances):
                if noisyDistance is not None:
                    emissionModel = busters.getObservationDistribution(noisyDistance)
                    weight *= emissionModel[util.manhattanDistance(pacmanPosition, particle[ghostIndex])]
            sampleWeights[particle] += weight
        # Resample
        if sampleWeights.totalCount() == 0:
            self.initializeParticles()
            self.moveNecessaryGhostsToJail(noisyDistances)
        else:
            self.particles = map(lambda _: util.sample(sampleWeights), self.particles)
    ...
    def moveNecessaryGhostsToJail(self, noisyDistances):
        for ghostIndex, noisyDistance in enumerate(noisyDistances):
            if noisyDistance == None:
                self.particles = map(lambda particle: self.getParticleWithGhostInJail(particle, ghostIndex), self.particles)
